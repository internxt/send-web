...
export const FilesProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    setTotalFilesCount(
      itemList.reduce((prev, current) => prev + (current.type === 'folder' ? current.countFiles || 0 : 1), 0),
    );
    setTotalFilesSize(itemList.reduce((prev, current) => prev + current.size, 0));
  }, [itemList]);
  ...
  const addFiles = (newFileList: FileWithPath[]) => {
    ...
    const newFilesTotalSize = newFileList.reduce((prev, current) => prev + current.size, 0);
    ...
    if (totalFilesSize + newFilesTotalSize <= MAX_BYTES_PER_SEND) {
      ...
      notificationsService.show({
        text: `The maximum size allowed is ${format(MAX_BYTES_PER_SEND)} in total`,
        type: ToastType.Warning,
      });
    }
  };
